...
context.configure({ // tells the canvas that you will recive data from this `device` in this `format` 
  device: device,
  format: canvasFormat,
});
...
const time = device.createBuffer({
  label: "Time buffer",
  size: 4,
...
if(!paused){ device.queue.writeBuffer(time, 0, timeBuffer); }
...
// document.getElementById("frametime").innerText = `FrameTime ${mean} ms`
}
